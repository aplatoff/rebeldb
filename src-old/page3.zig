...
/// Page
pub fn Page(comptime Capacity: comptime_int, comptime Offset: type, comptime Index: type) type {
    return packed struct {
        const Self = @This();
        ...
        pub fn init(self: *Self, _: usize) usize {
            self.len = 0;
            self.write = 0;
            return 0;
        }
        ...
        fn constIndexes(self: *const Self) []const Offset {
            const buffer = self.constBytes();
            const idx: [*]const Index align(1) = @alignCast(@ptrCast(&buffer[0]));
            return idx[0 .. buffer.len / @sizeOf(Index)];
        }

        fn indexes(self: *Self) []Offset {
            const buffer = self.bytes();
            const idx: [*]Index = @alignCast(@ptrCast(&buffer[0]));
            return idx[0 .. buffer.len / @sizeOf(Index)];
        }

        fn sizeOfIndexes(self: *const Self) Offset {
            return @sizeOf(Index) * (self.len + 1);
        }

        fn setIndex(self: *Self, index: Index, offset: Offset) void {
            const idx = self.indexes();
            idx[idx.len - 1 - index] = offset;
        }

        fn getIndex(self: *const Self, index: Index) Offset {
            const idx = self.constIndexes();
            return idx[idx.len - 1 - index];
        }

        fn get(self: *const Self, index: Index) [*]const u8 {
            const data = self.constBytes();
            return @ptrCast(&data[self.getIndex(index)]);
        }

        fn immediatelyAvailable(self: *const Self, size: usize) bool {
            const new_value = header + self.write.position() + size;
            const new_index = Capacity - self.sizeOfIndexes();
        ...
        fn bytes(self: *Self) []u8 {
            ...
            return ptr[header..Capacity];
        }

        pub inline fn available(self: *const Self, size: usize) bool {
            const new_value = header + self.write + size;
            const new_index = Capacity - self.sizeOfIndexes();
            return new_value <= new_index;
        }
        ...
        pub fn add(self: *Self, value: [*]const u8, size: Offset) void {
            ...
            // self.setIndex(self.len, pos);

            // const index_offset = Capacity - (self.len + 1) * @sizeOf(Index);
            // const ptr: *Offset = @alignCast(@ptrCast(&buf[index_offset]));
            // ptr.* = pos;

            const ptr: [*]Offset = @ptrCast(self);
            const total = Capacity / 2;
            const skip = header / @sizeOf(Index);
            const indices: []Offset = ptr[skip..total];
            indices[total - skip - self.len - 1] = pos;
        ...
        //         const cur_offset = self.getIndex(self.constBytes(), i);
