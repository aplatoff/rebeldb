...
const Cap = PageModule.ByteAligned(PageSize, u16);
...
// const PageType = PageModule.Page(PageSize, u16, u16);
...
fn benchAddEntries(allocator: std.mem.Allocator, iterations: usize) !void {
    ...
    for (0..iterations) |_| {
        ...
        while (page.available(entry_size)) {
            page.add(@ptrCast(&entry), entry_size);
        }
    }
}

fn benchAddEntriesMedium(allocator: std.mem.Allocator) void {
    benchAddEntries(allocator, LARGE_SIZE / 16) catch unreachable;
}
